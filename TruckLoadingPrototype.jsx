...
class DragController {
  constructor({ camera, domElement, truckDimensions, cargoRegistry, collisionSystem, onDragStateChange }) {
    this.camera = camera;
  ...
  onMove(e) {
    ...
    //point.x = THREE.MathUtils.clamp(
      //point.x,
      //-(TRUCK_DIMENSIONS.length / 2) + size.x / 2,
       //(TRUCK_DIMENSIONS.length / 2) - size.x / 2
    //);
    //point.z = THREE.MathUtils.clamp(
      //point.z,
      //-(TRUCK_DIMENSIONS.width / 2) + size.z / 2,
       //(TRUCK_DIMENSIONS.width / 2) - size.z / 2
    //);
    point.y = this.selectedEntry.mesh.position.y; 
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ lineHeight: '1.8' }}>
          <div><strong>Move box:</strong> Click + drag box</div>
