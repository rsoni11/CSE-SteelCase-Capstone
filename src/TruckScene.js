...
export function initScene({
  ...
  setIsBoxDragging,
  saveToHistoryRef,
  onGroupChanged,    // US6 Rhea — (ids: string[]) => void
}) {
  ...
  world.defaultContactMaterial = new CANNON.ContactMaterial(
    defaultMaterial, defaultMaterial, { friction: 0.95, restitution: 0.0 }
  );
  world.addContactMaterial(
    new CANNON.ContactMaterial(defaultMaterial, wallMaterial, { friction: 1.0, restitution: 0.0 })
  );

  physicsApiRef.current = { world, materials: { defaultMaterial, wallMaterial } };

  // ── Truck geometry ──────────────────────────────────────────────────────────
  const truckGroup = new THREE.Group();

  const floorGeometry = new THREE.BoxGeometry(TRUCK_DIMENSIONS.length, 0.2, TRUCK_DIMENSIONS.width);
  const floor = new THREE.Mesh(floorGeometry, new THREE.MeshStandardMaterial({ color: 0x8b7355, roughness: 0.8, metalness: 0.2 }));
  ...
  const frontWall = new THREE.Mesh(
    new THREE.BoxGeometry(0.2, TRUCK_DIMENSIONS.height, TRUCK_DIMENSIONS.width), visualWallMaterial
  );
  ...
  const ceiling = new THREE.Mesh(
    new THREE.BoxGeometry(TRUCK_DIMENSIONS.length, 0.2, TRUCK_DIMENSIONS.width), visualWallMaterial
  );
  ...
  addStaticBox(10, 0.08, 7.5, -40, 0.08, 0); // bay physics floor
  ...
  scene.add(ground);
  scene.add(new THREE.GridHelper(120, 60, 0xcccccc, 0xdddddd));
  ...
  return { scene, camera, renderer, dragController, cleanup };
}